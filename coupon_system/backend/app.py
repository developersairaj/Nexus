...
def get_available_coupons():
    ...
    try:
        ...
        return jsonify({'coupons': [serialize_coupon(c) for c in filtered_coupons]}), 200
...
def get_user_coupon_history():
    ...
    try:
        ...
        redemptions = CouponRedemption.query.filter_by(user_id=user_id)\
...
def get_coupon_analytics():
    ...
    try:
        ...
        ).join(CouponRedemption)\
         .filter(CouponRedemption.is_used == True)\
         .group_by(Coupon.id)\
         .order_by(db.func.count(CouponRedemption.id).desc())\
         .limit(10)
